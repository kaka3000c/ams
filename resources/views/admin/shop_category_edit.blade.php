...
<div class="main-div">
  <form action="/admin/shopcat/update" method="post" name="theForm" enctype="multipart/form-data" onsubmit="return validate()">
   {{ csrf_field() }}
   <table width="100%" id="general-table">
      <tr>
        <td class="label">分类名称:</td>
        <td>
          <input type='hidden' name='id' maxlength="20" value='{{$category['shop_cat_id']}}' size='27' />
        ...
        <td>
            <select name="parent_id" >
                     <option value="0">顶级分类</option>
                      ...
                      @if ($cate['shop_cat_id'] == $category['parent_id'] )
                      <option selected value="{{ $cate['shop_cat_id'] }}" >{{ $cate['cat_name'] }}</option>
                      @else
                      <option value="{{ $cate['shop_cat_id'] }}" >{{ $cate['cat_name'] }}</option>                      
                      @endif
                     @endforeach
                </select>
