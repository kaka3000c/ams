...
<table cellspacing='1' id="list-table">
    ...
    <td> <input name="chkGroup" type="checkbox" value="checkbox" onclick="check('{{$priv['priv_list']}}',this);" class="checkbox">{{ $priv['action_text'] }}  </td>
    
    <td>   
        @foreach ($priv['priv'] as $list)
         <label for="{{ $list['action_code'] }}"> <input type="checkbox" name="action_code[]" value="{{ $list['action_code'] }}" id="{{ $list['action_code'] }}" class="checkbox"/>
...
<div id="footer">
共执行 4 个查询，用时 0.045202 秒，Gzip 已禁用，内存占用 1.616 MB<br />
