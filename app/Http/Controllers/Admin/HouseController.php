...
class HouseController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $Array = DB::table('houses')->where('status',1)->orderBy('id', 'desc')->get()->toArray();
      ...
      $paginator =new LengthAwarePaginator($item, $totals, $perPage, $current_page, [
        ...
        return view('admin/house_list', ['info_list' => $paginator]);
    }
    ...
    public function on_show(Request $request,$id) {
        ...
        DB::table('houses')->where('id', $id)->update(['is_show' => 1]);
    }
    
    //信息下架
    public function off_show(Request $request,$id) {
        
        DB::table('houses')->where('id', $id)->update(['is_show' => 0]);
    }
    
    //放入回收站
    public function delete(Request $request,$id) {
        
        DB::table('houses')->where('id', $id)->update(['status' => 0]);
    }
    ...
    public function cancel_del(Request $request,$id) {
        ...
        DB::table('houses')->where('id', $id)->update(['status' => 1]);
    }
    
   //回收站商品列表
    public function trash(Request $request) {
        
       $Array = DB::table('houses')->where('status',0)->orderBy('id', 'desc')->get()->toArray(); 
        foreach ($Array as $key => $value) {
            $Array[$key] = (array)$value;
        }
