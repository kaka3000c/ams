...
class RoleController extends Controller {
    ...
    public function index(Request $request) {
        $Role = new Role;
        $role = $Role::paginate(2)->toArray();
         
        $action_list = ActionList::foo();
        print_r($action_list);
         foreach ($role['data'] as $key => $value) {
             echo $value['action_list'];
         if (array_key_exists($value['action_list'], $action_list))
                {
                       $role['data'][$key]['action_list'] = $action_list[$role['data'][$key]['action_list']];
              }
