...
class CarController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $Array = DB::table('cars')->where('status',1)->orderBy('id', 'desc')->get()->toArray();
      ...
      $paginator =new LengthAwarePaginator($item, $totals, $perPage, $current_page, [
        ...
        return view('admin/car_list', ['info_list' => $paginator]);
    }
    ...
    public function on_show(Request $request,$id) {
        ...
        DB::table('cars')->where('id', $id)->update(['is_show' => 1]);
    }
    
     //信息下架
    public function off_show(Request $request,$id) {
        
        DB::table('cars')->where('id', $id)->update(['is_show' => 0]);
    }
    
    //放入回收站
    public function delete(Request $request,$id) {
        
        DB::table('cars')->where('id', $id)->update(['status' => 0]);
    }
    ...
    public function real_del(Request $request,$id) {
            ...
            $promo_info = DB::table('cars')->where('id', $id)->first();
            ...
            $product = DB::table('cars')->where('id', $id)->delete();
    ...
    public function batch_del(Request $request) {
        ...
        $pro_id_array = $request->input('pro_id');
         ...
         for ($i=0; $i<count($pro_id_array); $i++) {
              DB::table('cars')->where('id', $pro_id_array[$i])->delete();
         }
          echo "批量删除成功";
    ...
    public function cancel_del(Request $request,$id) {
        ...
        DB::table('cars')->where('id', $id)->update(['status' => 1]);
    }
    
   //回收站商品列表
    public function trash(Request $request) {
         $Array = DB::table('cars')->where('status',0)->orderBy('id', 'desc')->get()->toArray(); 
       foreach ($Array as $key => $value) {
            $Array[$key] = (array)$value;
        }
