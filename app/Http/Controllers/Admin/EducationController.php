...
class EducationController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $Array = DB::table('education')->where('status',1)->orderBy('id', 'desc')->get()->toArray();
      ...
      $paginator =new LengthAwarePaginator($item, $totals, $perPage, $current_page, [
        ...
        return view('admin/education_list', ['info_list' => $paginator]);
    }
    ...
    public function on_show(Request $request,$id) {
        ...
        DB::table('education')->where('id', $id)->update(['is_show' => 1]);
    }
    
    //取消回收站
    public function off_show(Request $request,$id) {
        
        DB::table('education')->where('id', $id)->update(['is_show' => 0]);
    }
    
    //放入回收站
    public function delete(Request $request,$id) {
        
        DB::table('education')->where('id', $id)->update(['status' => 0]);
    }
   ...
   //回收站商品列表
    public function trash(Request $request) {
      ...
      $Array = DB::table('education')->where('status',0)->orderBy('id', 'desc')->get()->toArray(); 
      foreach ($Array as $key => $value) {
            $Array[$key] = (array)$value;
        }
        //分页
