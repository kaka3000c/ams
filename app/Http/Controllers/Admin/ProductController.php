...
class ProductController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $Array = DB::table('products')->where('is_delete',0)->orderBy('pro_id', 'desc')->get()->toArray();
    ...
    public function update(Request $request) {
        ...
        DB::table('products')->where('pro_id', $pro_id)->update(['goods_name' => $goods_name
                ,'content' => $content , 'shop_price'=> $shop_price
            ]);
        
        if ($request->hasFile('goods_img')) {
             DB::table('prodcuts')->where('pro_id', $pro_id)
   ...
   //回收站商品列表
    public function trash(Request $request) {
      ...
      $paginator =new LengthAwarePaginator($item, $totals, $perPage, $current_page, [
        ...
        return view('admin/product_trash', ['product_list' => $paginator]);
    }
